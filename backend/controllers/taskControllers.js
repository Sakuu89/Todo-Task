...
exports.putTask = async (req, res) => {
  try {
    ...
    if (!description && completed === undefined) { // Check if either description or completed is provided
      return res.status(400).json({ status: false, msg: "Description or completed status of task not found" });
    }
    ...
    const updatedFields = {}; // Object to store fields to update
    if (description) {
    ...
    task = await Task.findByIdAndUpdate(req.params.taskId, updatedFields, { new: true });
    res.status(200).json({ task, status: true, msg: "Task updated successfully.." });
  }
  catch (err) {
    console.error(err);
